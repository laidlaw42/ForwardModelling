# TASK 2.4
# Run the simulation to year 100 with one dyke hotter than the other.
...
T2 = 800  # initial temperature dyke 2 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< dyke 2 @ 800
...
plt.figure(1)
plt.title(label='Temperature profile at 100 years, dyke 1 = 1200⁰C, dyke 2 = ' + str(T2) + '⁰C')
plt.plot(x, T)
plt.xlabel("Position")
...
Tc[0] = T0
plt.figure(2)
plt.title(label='Temperature evolution at 100 years, dyke 1 = 1200⁰C, dyke 2 = ' + str(T2) + '⁰C')
plt.plot(tc[:-2], Tc[:-2])
plt.xlabel("Time")
plt.ylabel("Temperature")
plt.show()
